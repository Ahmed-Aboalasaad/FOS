...
void TransferUserPage(char** arguments)
{
	...
	// Get the source to point to the first byte in its page
	src /= PAGE_SIZE;
	src *= PAGE_SIZE;

	// Find the source & destination pages borders
	uint32 *src_PT = NULL;
	get_page_table(ptr_page_directory, src, 1, &src_PT);
	uint32 *src_first_entry = &src_PT[0];
	uint32 *src_last_entry = &src_PT[1023];

	uint32 *dst_PT = NULL;
	get_page_table(ptr_page_directory, dst, 1, &dst_PT);
	uint32 *dst_first_entry = &dst_PT[0];

	if (strcmp(mode, "c") == 0) { // Copy
		for (uint32 *i = src_first_entry; i <= src_last_entry; i++) {
			
		}
	}
	else if (strcmp(mode, "m") == 0) { // move

	}
	else
		cprintf("Usage: tup <source va in HEX> <dest va in HEX> <transfer mode (c/m)>\n");
}
//========================================================
